...
class Seq2SQL_v2(nn.Module):
    ...
    def beam_forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, engine, tb,
        ...
        # wemb_hpu : torch.Size([6, 4, 1536]). 4 is the max length of sub words.
        # l_hpu :[1, 2, 1, 1, 3, 4]
        ...
        idxs = topk_multi_dim(torch.tensor(prob_sca), n_topk=beam_size, batch_exist=True) # [[[0, 0], [1, 0], [1, 1], [1, 2]]]
        # Now as sc_idx is already sorted, re-map them properly.

        idxs = remap_sc_idx(idxs, pr_sc_beam) # [sc_beam_idx, sa_idx] -> [sc_idx, sa_idx]. [[[3, 0], [1, 0], [1, 1], [1, 2]]]
        idxs_arr = array(idxs)
        # [B, beam_size, remainig dim] = [16,4,2] or [1,4,2] 4 best options for sc & sa (2).
        # idxs[b][0] gives the highest score [sc_idx, sa_idx] pairs.
        # idxs[b][1] gives of second highest score.

        ### If you want to return top n select and agg, here is what you can use.

        # Calculate prob_sca, a joint probability
        beam_idx_sca = [0] * bS
        beam_meet_the_final = [False] * bS
        while True:
            pr_sc = idxs_arr[range(bS),beam_idx_sca,0] #[3]
            pr_sa = idxs_arr[range(bS),beam_idx_sca,1] #[0]
            ...
            if sum(check) == bS:
                ...
                for b, check1 in enumerate(check):
                    if not check1: # wrong pair
                        #import pdb;pdb.set_trace()
                        beam_idx_sca[b] += 1 # This means pr_sc & pr_sa index will be updated accordingly to the next highest candidate.
...
class WVP_se(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn, wc, wo, wenc_n=None, show_p_wv=False, g_wvi=None):
        ...
        # list to [B, 4, dim] tensor.
        wenc_hs_ob = torch.stack(wenc_hs_ob)  # list to tensor.
        wenc_hs_ob = wenc_hs_ob[:,:self.mL_w,:].to(device)
        ...
        #  --> [B, 4, mL_n, dim]
        #  --> [B, 4, dim= 100] 
        c_n = torch.mul(wenc_n.unsqueeze(1), p.unsqueeze(3)).sum(dim=2)[:,:self.mL_w,:] # [1, 6, 100]
