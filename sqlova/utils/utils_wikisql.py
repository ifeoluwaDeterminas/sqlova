...
def str_ix_to_tok_ix(wv_str_ix, nlu, nlu_t, after):
	"""
	input  : list of list string index. e.i. [[12,11,'3D printers'],[40,9,'Microsoft']]
	output : list of list tokens. [[[2, 3], [6, 6]]]

	"""
	nlu = nlu[0]
	nlu_t = nlu_t[0]
	str_ix = wv_str_ix[0] # TODO: Later consider when the batch size is not 1.
	tok_idx=[]
	#import pdb;pdb.set_trace()
	...
	for val in str_ix :
		...
		if value1 in nlu_t: # check if true/false
			index = nlu_t.index(value1)
			tok_idx.append([index,index])
		# case2: 2 tokens
		else:
			nlu_t2 = nlu_t[1:]
			nlu_t2.append("")
			...
			if value1 in two_tok_list and nlu_t[ix1]+after[ix1]+nlu_t[ix1+1] == value1:
				...
				after2 = after[1:]
				after2.append("")
				tri_tok_list = [ t + _ + t2 + __ + t3 for t, _, t2, __, t3 in zip(nlu_t, after, nlu_t2, after2, nlu_t3)]
				ix1 = nlu_t.index(tok_temp[0])
				
				if value1 in tri_tok_list and nlu_t[ix1]+after[ix1]+nlu_t2[ix1]+after2[ix1] + nlu_t3[ix1] == value1:
					tok_idx.append([ix1, ix1+2])

		#else: #TODO: treat more than 3 tokens in an elegant way
		#	tok_idx.append([None])
	return [tok_idx]